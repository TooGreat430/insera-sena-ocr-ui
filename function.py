...
def run_ocr(invoice_name, uploaded_pdf_paths, with_total_container):
    ...
    detail_csv = _call_gemini(merged_pdf, DETAIL_SYSTEM_INSTRUCTION)
    bucket.blob(f"{RESULT_PREFIX}/detail/{invoice_name}_detail.csv").upload_from_string(detail_csv)

    if with_total_container:
        total_csv = _call_gemini(merged_pdf, TOTAL_SYSTEM_INSTRUCTION)
        container_csv = _call_gemini(merged_pdf, CONTAINER_SYSTEM_INSTRUCTION)
        bucket.blob(f"{RESULT_PREFIX}/total/{invoice_name}_total.csv").upload_from_string(total_csv)
        bucket.blob(f"{RESULT_PREFIX}/container/{invoice_name}_container.csv").upload_from_string(container_csv)
